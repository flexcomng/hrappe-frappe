...
def get_all(
    ...
    try:
        ...
        if not filters:
            filters = {}
        data = frappe.get_all(
            doctype, fields, filters, order_by, group_by, start, page_length
        )
        return generate_response("S", "200", message="Success", data=data)
    except Exception as e:
        return generate_response("F", error=e)
...
@frappe.whitelist()
def make_payment_si(
    docname=None, ref_type=None, ref_no=None, amount=None, power_data=None
):
    if not docname:
        return generate_response("F", error="'docname' parameter is required")
    if not ref_type:
    ...
    try:
        payment_doc = get_payment_entry("Sales Invoice", docname)
        payment_doc.update(
            {
